...
func Update(delta: float):
	...
	if not punchReleased and animator.animation == "ChargePunch":
		if animator.frame >= 8:
			_charge_frame_timer += delta
...
func _on_animation_finished():
	if animator.animation == "Right Punch":
		hitbox.disable()
		if chargeLevel > 0:
			if canShoot:
				shoot_projectile()
...
func Physics_Update(delta: float):
	...
	player.velocity = direction * speed if direction != Vector2.ZERO else Vector2.ZERO
