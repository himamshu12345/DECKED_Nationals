...
func _on_area_entered(area: Area2D) -> void:
	if area is HitBox:
		...
		# --- Stagger-loop guard ---
		# If the owner is already in a stagger / exhausted / death state, do not
		# process the hit at all.  This prevents boss punches from resetting the
		# ConfusedStaggered animation and trapping the player indefinitely.
		if owner and owner.has_node("StateMachine"):
			var owner_state: String = owner.get_node("StateMachine").current_state.name
